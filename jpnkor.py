...
st.markdown(
    "검색하고, 예문마다 **일본어 문장만 깔끔하게** 원어민 음성으로 들어보세요!"
)
...
if st.session_state.current_result:
  ...
  if ex_matches:
    ...
    for num, content in ex_matches:
      ...
      # 2. '[일본어 원문]' 같은 제목/라벨 텍스트와 마크다운 기호 제거
      clean_jp = re.sub(
          r"(\[일본어 원문\]|일본어 원문[:]?|\*\*일본어 원문\*\*[:]?|원문)",
          "",
          clean_jp,
      )
      # 3. 괄호, 슬래시, 대시 등 읽기나 뜻이 시작되는 기호 기준으로 뒷부분 잘라내기
      clean_jp = re.split(r"[\(\（\/\-\–\—\|\:]", clean_jp)[0]
      # 4. 남은 마크다운 기호(*, #, _, `, ~ 등) 제거 후 공백 정리
