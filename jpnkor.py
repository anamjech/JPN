...
def perform_search(q):
  ...
  prompt = f"""
    ...
    5. **훈독** (없으면 '없음')
    6. **음독** (없으면 '없음')
    7. **실생활 예문 3개** (간단하지만 자주 쓰는 문장으로 구성하되, 반드시 아래 형식을 정확히 지켜주세요. 특히 [히라가나 읽기]는 한국어 발음 표기가 아닌 **오직 히라가나 원문**으로만 작성해 주세요.)
...
if st.session_state.current_result:
  ...
  if ex_matches:
    ...
    for num, content in ex_matches:
      ...
      # [일본어 원문] 라벨이 포함된 줄에서만 순수 원문 텍스트를 정확하게 추출
      clean_jp = ""
      for line in content.split("\n"):
        if "일본어 원문" in line:
          # [일본어 원문] 뒤쪽의 내용만 가져옴
          text_part = re.sub(r".*?\[일본어 원문\]\s*[:]?\s*", "", line)
          # 마크다운 기호 제거
