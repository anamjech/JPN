...
def perform_search(q):
  ...
  prompt = f"""
    ...
    2. **한자** (한자가 없는 단어면 '음표시' 대신 '없음' 표시)
    3. **히라가나**
    4. **한글 뜻**
    5. **훈독** (없으면 '없음')
    6. **음독** (없으면 '없음')
    7. **실생활 예문 3개** (간단하지만 자주 쓰는 문장으로 구성하되, 일본어 원문과 읽기, 한글 뜻을 포함해 자연스럽고 보기 좋게 작성해 주세요. 
...
st.markdown(
    "검색하고, 예문마다 **일본어 원문만 깔끔하게** 원어민 음성으로 들어보세요!"
)
...
if st.session_state.current_result:
  ...
  if ex_matches:
    ...
    for num, content in ex_matches:
      ...
      # 2. 괄호나 대시 등 부가 설명이 시작되기 전 앞부분만 추출
      clean_jp = re.split(r"[\(\（\/\-\–\—]", clean_jp)[0]
      # 3. 오직 순수 일본어 문자(한자, 히라가나, 가타카나)만 정확히 골라냄 (한국어/기호 완벽 차단)
      jp_chars = re.findall(
          r"[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF\u30FC\u3005]+", clean_jp
      )
