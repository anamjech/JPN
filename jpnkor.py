...
st.markdown(
    "검색하고, 예문마다 **일본어 원문 문장만 깔끔하게** 원어민 음성으로"
    " 들어보세요!"
)
...
if st.session_state.current_result:
  ...
  if ex_matches:
    ...
    for num, content in ex_matches:
      ...
      # 2. '일본어 원문' 같은 불필요한 라벨 및 마크다운 특수문자 제거
      clean_jp = re.sub(
          r"(일본어 원문|원문|[\*\#\_\-\`\~\[\]])", "", clean_jp
      )
      # 3. 괄호, 슬래시, 대시 또는 한글(한글 뜻, 히라가나 읽기 영역)이 나오는 순간 그 뒤는 무조건 잘라냄!
      clean_jp = re.split(r"[\(\（\/\-\–\—\|\:\uAC00-\uD7A3]", clean_jp)[0]
      # 4. 남은 공백 정리
      clean_jp = clean_jp.strip()
